...
#define CLAY_TEXT(text, ...)                                                                       \
  Clay__OpenTextElement(text, CLAY__CONFIG_WRAPPER(Clay_TextElementConfig, __VA_ARGS__))
...
typedef struct Clay_String {
  ...
  int32_t length;
  // The underlying character memory. Note: this will not be copied and will not extend the lifetime
...
typedef struct Clay_SizingAxis {
  union {
    Clay_SizingMinMax
        minMax; // Controls the minimum and maximum size in pixels that this element is allowed to
                // grow or shrink to, overriding sizing types such as FIT or GROW.
    float percent; // Expects 0-1 range. Clamps the axis size to a percent of the parent container's
                   // axis size minus padding and child gaps.
  } size;
...
typedef struct Clay_LayoutConfig {
  Clay_Sizing sizing; // Controls the sizing of this element inside it's parent container, including
                      // FIT, GROW, PERCENT and FIXED sizing.
  Clay_Padding padding; // Controls "padding" in pixels, which is a gap between the bounding box of
                        // this element and where its children will be placed.
...
typedef struct Clay_TextElementConfig {
  ...
  uint16_t lineHeight;
  // Controls how text "wraps", that is how it is broken into multiple lines when there is
  // insufficient horizontal space. CLAY_TEXT_WRAP_WORDS (default) breaks on whitespace characters.
  // CLAY_TEXT_WRAP_NEWLINES doesn't break on space characters, only on newlines.
  // CLAY_TEXT_WRAP_NONE disables wrapping entirely.
  Clay_TextElementConfigWrapMode wrapMode;
  // Controls how wrapped lines of text are horizontally aligned within the outer text bounding box.
  // CLAY_TEXT_ALIGN_LEFT (default) - Horizontally aligns wrapped lines of text to the left hand
  // side of their bounding box. CLAY_TEXT_ALIGN_CENTER - Horizontally aligns wrapped lines of text
  // to the center of their bounding box. CLAY_TEXT_ALIGN_RIGHT - Horizontally aligns wrapped lines
  // of text to the right hand side of their bounding box.
...
typedef struct Clay_AspectRatioElementConfig {
  float aspectRatio; // A float representing the target "Aspect ratio" for an element, which is its
                     // final width divided by its final height.
...
typedef struct Clay_FloatingElementConfig {
  ...
  // the other element with .id = CLAY_ID("yourId"), and specify the id the same way, with .parentId
  // = CLAY_ID("yourId").id
  uint32_t parentId;
  ...
  // elements underneath / behind a floating element. Enum is of the form CLAY_ATTACH_POINT_foo_bar.
  // See Clay_FloatingAttachPoints for more details. Note: see <img
  ...
  // Controls how mouse pointer events like hover and click are captured or passed through to
  // elements underneath a floating element. CLAY_POINTER_CAPTURE_MODE_CAPTURE (default) - "Capture"
  // the pointer event and don't allow events like hover and click to pass through to elements
  // underneath. CLAY_POINTER_CAPTURE_MODE_PASSTHROUGH - Transparently pass through pointer events
  // like hover and click to elements underneath the floating element.
  ...
  // element to an element with a specific ID, specified with the .parentId field. positioned based
  // on the .attachPoints and .offset fields. CLAY_ATTACH_TO_ROOT - Attaches this floating element
  // to the root of the layout, which combined with the .offset field provides functionality similar
...
typedef struct Clay_BorderWidth {
  ...
  // horizontal lines. .betweenChildren borders will result in individual RECTANGLE render commands
  // being generated.
...
typedef struct Clay_BorderElementConfig {
  Clay_Color color; // Controls the color of all borders with width > 0. Conventionally represented
                    // as 0-255, but interpretation is up to the renderer.
  Clay_BorderWidth width; // Controls the widths of individual borders. At least one of these should
                          // be > 0 for a BORDER render command to be generated.
...
typedef struct Clay_TextRenderData {
  ...
  Clay_Color textColor;
  // An integer representing the font to use to render this text, transparently passed through from
...
typedef struct Clay_RectangleRenderData {
  // The solid background color to fill this rectangle with. Conventionally represented as 0-255 for
  // each channel, but interpretation is up to the renderer.
  Clay_Color backgroundColor;
...
typedef struct Clay_ImageRenderData {
  ...
  // radius) pixels.
  Clay_CornerRadius cornerRadius;
  // A pointer transparently passed through from the original element definition, typically used to
...
// Render command data when commandType == CLAY_RENDER_COMMAND_TYPE_OVERLAY_COLOR_START ||
...
typedef struct Clay_ScrollContainerData {
  // Note: This is a pointer to the real internal scroll position, mutating it may cause a change in
  // final layout. Intended for use with external functionality that modifies scroll position, such
  // as scroll bars or auto scrolling.
  Clay_Vector2 *scrollPosition;
  // The bounding box of the scroll element.
  Clay_Dimensions scrollContainerDimensions;
  // The outer dimensions of the inner scroll container content, including the padding of the parent
  // scroll container.
...
typedef CLAY_PACKED_ENUM{
    ...
    // full again.
    CLAY_RENDER_COMMAND_TYPE_SCISSOR_END,
    // The renderer should begin performing a "color overlay" on all subsequent render commands
    // until disabled again.
    CLAY_RENDER_COMMAND_TYPE_OVERLAY_COLOR_START,
    // The renderer should disable any previously active "color overlay" and render elements with
    // their standard colors again.
...
typedef struct Clay_PointerData {
  ...
  // CLAY_POINTER_DATA_PRESSED - The left mouse button click or touch happened at some point in the
  // past, and is still currently held down this frame. CLAY_POINTER_DATA_RELEASED_THIS_FRAME - The
  // left mouse button click or touch was released this frame. CLAY_POINTER_DATA_RELEASED - The left
  // mouse button click or touch is not currently down / was released at some point in the past.
...
typedef struct Clay_ElementDeclaration {
  ...
  Clay_Color backgroundColor;
  // Perform an image editing style "Color Overlay" on this element and all its children, equivalent
  // to glsl mix(elementColor, overlayColor.rgb, overlayColor.a)
  Clay_Color overlayColor;
  // Controls the "radius", or corner rounding of elements, including rectangles, borders and
  // images.
  Clay_CornerRadius cornerRadius;
  ...
  // Note: in order to activate floating, .floating.attachTo must be set to something other than the
  // default value.
...
typedef struct Clay_ErrorData {
  ...
  // CLAY_ERROR_TYPE_ELEMENTS_CAPACITY_EXCEEDED - Clay ran out of capacity in its internal array for
  // storing elements. This limit can be increased with Clay_SetMaxElementCount().
  ...
  // provided .parentId was found. CLAY_ERROR_TYPE_PERCENTAGE_OVER_1 - An element was declared that
  // using CLAY_SIZING_PERCENT but the percentage value was over 1. Percentage values are expected
  // to be in the 0-1 range. CLAY_ERROR_TYPE_INTERNAL_ERROR - Clay encountered an internal error. It
  // would be wonderful if you could report this so we can fix it!
  // CLAY_ERROR_TYPE_UNBALANCED_OPEN_CLOSE - Clay__OpenElement was called more times than
  // Clay__CloseElement, so there were still remaining open elements when the layout ended.
  // CLAY_ERROR_TYPE_HASH_MAP_CAPACITY_EXCEEDED - Clay ran out of capacity in its internal hash map
  // for storing element IDs -> elements. This limit can be increased with
...
// Returns the state of the "pointer" (i.e. the mouse or touch) which was set via
...
#define CLAY__ARRAY_DEFINE_FUNCTIONS(typeName, arrayName)                                          \
  ...
  typeName *arrayName##_GetCheckCapacity(arrayName *array, int32_t index) {                        \
    return Clay__Array_RangeCheck(index, array->capacity) ? &array->internalArray[index]           \
                                                          : &typeName##_DEFAULT;                   \
  }                                                                                                \
                                                                                                   \
...
bool Clay__BorderHasAnyWidth(Clay_BorderElementConfig *borderConfig) {
  return borderConfig->width.betweenChildren > 0 || borderConfig->width.left > 0 ||
         borderConfig->width.right > 0 || borderConfig->width.top > 0 ||
...
Clay__MeasureTextCacheItem *Clay__MeasureTextCached(Clay_String *text,
    ...
    if (context->measureTextHashMapInternal.length ==
      ...
      if (!context->booleanWarnings.maxTextMeasureCacheExceeded) {
        context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData){
            ...
            .errorText =
                CLAY_STRING("Clay ran out of capacity while attempting to measure text elements. "
  ...
  while (end < text->length) {
    if (context->measuredWords.length == context->measuredWords.capacity - 1) {
      if (!context->booleanWarnings.maxTextMeasureCacheExceeded) {
        context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData){
            ...
            .errorText = CLAY_STRING("Clay has run out of space in it's internal text measurement "
    ...
    if (current == ' ' || current == '\n') {
      ...
      if (length > 0) {
        dimensions = Clay__MeasureText(CLAY__INIT(Clay_StringSlice){.length = length,
...
Clay_LayoutElementHashMapItem *Clay__AddHashMapItem(Clay_ElementId elementId,
  ...
  if (context->layoutElementsHashMapInternal.length ==
    ...
    if (!context->booleanWarnings.hashMapCapacityExceeded) {
      context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData){
          ...
          .errorText = CLAY_STRING("Clay has run out of space in it's internal element ID hashmap. "
                                   " Try using Clay_SetMaxElementCount() with a higher value."),
          .userData = context->errorHandler.userData});
  ...
  while (hashItemIndex !=
    ...
    if (hashItem->elementId.id == elementId.id) { // Collision - resolve based on generation
      ...
      if (hashItem->generation <=
        ...
        hashItem->elementId =
            elementId; // Make sure to copy this across. If the stringId reference has changed, we
  ...
  if (context->layoutElementsHashMapFreeList.length > 0) {
    indexToUse = Clay__int32_tArray_GetValue(&context->layoutElementsHashMapFreeList,
  ...
  Clay_LayoutElementHashMapItem *hashItem = Clay__LayoutElementHashMapItemArray_Set(
  ...
  if (layoutElement->config.aspectRatio.aspectRatio != 0) {
    if (layoutElement->dimensions.width == 0 && layoutElement->dimensions.height != 0) {
      layoutElement->dimensions.width =
          layoutElement->dimensions.height * layoutElement->config.aspectRatio.aspectRatio;
    } else if (layoutElement->dimensions.width != 0 && layoutElement->dimensions.height == 0) {
      layoutElement->dimensions.height =
          layoutElement->dimensions.width * (1 / layoutElement->config.aspectRatio.aspectRatio);
    }
  }
}
...
void Clay__CloseElement(void) {
  ...
  if (layoutConfig->layoutDirection == CLAY_LEFT_TO_RIGHT) {
    ...
    for (int32_t i = 0; i < openLayoutElement->children.length; i++) {
      ...
      // Minimum size of child elements doesn't matter to clip containers as they can shrink and
      // hide their contents
    ...
    float childGap =
        (float)(CLAY__MAX(openLayoutElement->children.length - 1, 0) * layoutConfig->childGap);
  ...
  if (layoutConfig->sizing.width.type != CLAY__SIZING_TYPE_PERCENT) {
    ...
    openLayoutElement->dimensions.width = CLAY__MIN(
        CLAY__MAX(openLayoutElement->dimensions.width, layoutConfig->sizing.width.size.minMax.min),
...
void Clay__OpenTextElement(Clay_String text, Clay_TextElementConfig textConfig) {
  ...
  Clay_ElementId elementId = Clay__HashNumber(
      parentElement->children.length + parentElement->floatingChildrenCount, parentElement->id);
...
void Clay__ConfigureOpenElementPtr(const Clay_ElementDeclaration *declaration) {
  ...
  if ((declaration->layout.sizing.width.type == CLAY__SIZING_TYPE_PERCENT &&
    ...
    context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData){
        ...
        .errorText = CLAY_STRING(
            "An element was configured with CLAY_SIZING_PERCENT, but the provided percentage value "
            "was over 1.0. Clay expects a value between 0 and 1, i.e. 20% is 0.2."),
        .userData = context->errorHandler.userData});
  }
  ...
  if (declaration->floating.attachTo != CLAY_ATTACH_TO_NONE) {
    ...
    // This looks dodgy but because of the auto generated root element the depth of the tree will
    // always be at least 2 here
      ...
      if (declaration->floating.attachTo == CLAY_ATTACH_TO_PARENT) {
        ...
        if (context->openClipElementStack.length > 0) {
          clipElementId = Clay__int32_tArray_GetValue(
              &context->openClipElementStack, (int)context->openClipElementStack.length - 1);
        }
        ...
        if (parentItem == &Clay_LayoutElementHashMapItem_DEFAULT) {
          context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData){
              ...
              .errorText = CLAY_STRING("A floating element was declared with a parentId, but no "
...
void Clay__SizeContainersAlongAxis(bool xAxis, float deltaTime, Clay__int32_tArray *textElementsOut,
  ...
  for (int32_t rootIndex = 0; rootIndex < context->layoutElementTreeRoots.length; ++rootIndex) {
    ...
    for (int32_t i = 0; i < bfsBuffer.length; ++i) {
      ...
      float parentPadding =
          (float)(xAxis ? (parentLayoutConfig->padding.left + parentLayoutConfig->padding.right)
      ...
      bool sizingAlongAxis = (xAxis && parentLayoutConfig->layoutDirection == CLAY_LEFT_TO_RIGHT) ||
      ...
      for (int32_t childOffset = 0; childOffset < parent->children.length; childOffset++) {
        ...
        if (sizingAlongAxis) {
          ...
          if (!isFirstChild) {
            innerContentSize += parentChildGap; // For children after index 0, the childAxisOffset
                                                // is the gap from the previous child
      ...
      if (sizingAlongAxis) {
        ...
        if (sizeToDistribute < 0) {
          // If the parent clips content in this axis direction, don't compress children, just leave
          // them alone
        ...
        for (int32_t childOffset = 0; childOffset < resizableContainerBuffer.length;
          ...
          Clay_SizingAxis childSizing = Clay__GetElementSizing(childElement, xAxis);
          float minSize =
              xAxis ? childElement->minDimensions.width : childElement->minDimensions.height;
          float *childSize =
              xAxis ? &childElement->dimensions.width : &childElement->dimensions.height;

          float maxSize = parentSize - parentPadding;
          // If we're laying out the children of a scroll panel, grow containers expand to the size
          // of the inner content, not the outer container
          if (((xAxis && parent->config.clip.horizontal) ||
               (!xAxis && parent->config.clip.vertical))) {
  ...
  if (context->renderCommands.length < context->renderCommands.capacity - 1) {
    ...
    if (!context->booleanWarnings.maxRenderCommandsExceeded) {
      ...
      context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData){
          .errorType = CLAY_ERROR_TYPE_ELEMENTS_CAPACITY_EXCEEDED,
          .errorText = CLAY_STRING(
              "Clay ran out of capacity while attempting to create render commands. This is "
...
void Clay__CalculateFinalLayout(float deltaTime, bool useStoredBoundingBoxes,
  ...
  for (int32_t textElementIndex = 0; textElementIndex < textElements.length; ++textElementIndex) {
    Clay_LayoutElement *element = Clay_LayoutElementArray_Get(
        &context->layoutElements, Clay__int32_tArray_GetValue(&textElements, textElementIndex));
    ...
    Clay__MeasureTextCacheItem *measureTextCacheItem =
        Clay__MeasureTextCached(&textElementData->text, &containerElement->textConfig);
    float lineWidth = 0;
    ...
    while (wordIndex != -1) {
      ...
      // measuredWord->length == 0 means a newline character
        ...
        bool finalCharIsSpace =
            textElementData->text.chars[CLAY__MAX(lineStartOffset + lineLengthChars - 1, 0)] == ' ';
        Clay__WrappedTextLineArray_Add(
            &context->wrappedTextLines,
            CLAY__INIT(Clay__WrappedTextLine){
    ...
    if (lineLengthChars > 0) {
      Clay__WrappedTextLineArray_Add(
          &context->wrappedTextLines,
          CLAY__INIT(Clay__WrappedTextLine){
              {lineWidth - containerElement->textConfig.letterSpacing, lineHeight},
  ...
  for (int32_t i = 0; i < aspectRatioElements.length; ++i) {
    Clay_LayoutElement *aspectElement = Clay_LayoutElementArray_Get(
        &context->layoutElements, Clay__int32_tArray_GetValue(&aspectRatioElements, i));
    aspectElement->dimensions.height =
        (1 / aspectElement->config.aspectRatio.aspectRatio) * aspectElement->dimensions.width;
  ...
  while (dfsBuffer.length > 0) {
    ...
    if (!context->treeNodeVisited.internalArray[dfsBuffer.length - 1]) {
      ...
      // Add the children to the DFS buffer (needs to be pushed in reverse so that stack traversal
      // is in correct layout order)
    ...
    if (layoutConfig->layoutDirection == CLAY_LEFT_TO_RIGHT) {
      ...
      contentHeight +=
          (float)(CLAY__MAX(currentElement->children.length - 1, 0) * layoutConfig->childGap);
      currentElement->dimensions.height =
  ...
  for (int32_t i = 0; i < aspectRatioElements.length; ++i) {
    ...
    aspectElement->dimensions.width =
        aspectElement->config.aspectRatio.aspectRatio * aspectElement->dimensions.height;
  }
  ...
  for (int32_t rootIndex = 0; rootIndex < context->layoutElementTreeRoots.length; ++rootIndex) {
    ...
    if (rootElement->config.floating.attachTo != CLAY_ATTACH_TO_NONE && parentHashMapItem) {
      ...
      switch (config->attachPoints.parent) { // I know I could merge the x and y switch statements,
    ...
    if (root->clipElementId) {
      ...
      if (clipHashMapItem && !Clay__ElementIsOffscreen(&clipHashMapItem->boundingBox)) {
        // Floating elements that are attached to scrolling contents won't be correctly positioned
        // if external scroll handling is enabled, fix here
        if (context->externalScrollHandlingEnabled) {
        ...
        if (generateRenderCommands) {
          Clay__AddRenderCommand(CLAY__INIT(Clay_RenderCommand){
              ...
              .userData = 0,
              .id = Clay__HashNumber(rootElement->id, rootElement->children.length + 10)
    ...
    Clay__LayoutElementTreeNodeArray_Add(
        &dfsBuffer, CLAY__INIT(Clay__LayoutElementTreeNode){
                        ...
                        .nextChildOffset = {.x = (float)rootElement->config.layout.padding.left,
    ...
    while (dfsBuffer.length > 0) {
      ...
      Clay_LayoutConfig *layoutConfig =
          currentElement->isTextElement ? &CLAY_LAYOUT_DEFAULT : &currentElement->config.layout;
            ...
            if (borderConfig->width.betweenChildren > 0 && borderConfig->color.a > 0) {
              ...
              if (layoutConfig->layoutDirection == CLAY_LEFT_TO_RIGHT) {
                for (int32_t i = 0; i < currentElement->children.length; ++i) {
                  ...
                  if (i > 0) {
                    Clay__AddRenderCommand(CLAY__INIT(Clay_RenderCommand){
                        .boundingBox = {currentElementBoundingBox.x + borderOffset.x +
                        ...
                        .id = Clay__HashNumber(currentElement->id,
                                               currentElement->children.length + 1 + i)
                                  .id,
      ...
      if (!currentElement->isTextElement) {
        ...
        if (currentElement->config.clip.horizontal || currentElement->config.clip.vertical) {
          // This linear scan could theoretically be slow under very strange conditions, but I can't
          // imagine a real UI with more than a few 10's of scroll containers
          for (int32_t i = 0; i < context->scrollContainerDatas.length; i++) {
            Clay__ScrollContainerDataInternal *mapping =
                Clay__ScrollContainerDataInternalArray_Get(&context->scrollContainerDatas, i);
            if (mapping->layoutElement == currentElement) {
      ...
      if (generateRenderCommands && !offscreen) {
        if (currentElement->isTextElement) {
                ...
                .renderData = {.text =
                                   {
                                       .stringContents =
                                           CLAY__INIT(Clay_StringSlice){
                                               ...
                                               .baseChars =
                                                   currentElement->textElementData.text.chars},
          ...
          if (currentElement->config.overlayColor.a > 0) {
            Clay_RenderCommand renderCommand = {
                .renderData = {.overlayColor = {.color = currentElement->config.overlayColor}},
                .userData = currentElement->config.userData,
                .id = currentElement->id,
                .zIndex = root->zIndex,
      ...
      if (layoutConfig->layoutDirection == CLAY_LEFT_TO_RIGHT) {
        for (int32_t i = 0; i < currentElement->children.length; ++i) {
          ...
          contentSizeCurrent.height =
              CLAY__MAX(contentSizeCurrent.height, childElement->dimensions.height);
        }
        ...
        for (int32_t i = 0; i < currentElement->children.length; ++i) {
          ...
          if (childElement->exiting)
            continue;
          contentSizeCurrent.width =
              CLAY__MAX(contentSizeCurrent.width, childElement->dimensions.width);
      ...
      for (int32_t i = 0; i < currentElement->children.length; ++i) {
        ...
        Clay_Vector2 childPosition = {
            currentElementBoundingBox.x + currentElementTreeNode->nextChildOffset.x +
                scrollOffset.x,
            currentElementBoundingBox.y + currentElementTreeNode->nextChildOffset.y +
                scrollOffset.y,
        };
        ...
        dfsBuffer.internalArray[newNodeIndex] = CLAY__INIT(Clay__LayoutElementTreeNode){
            ...
            .nextChildOffset = {.x = (float)childElement->config.layout.padding.left,
    ...
    if (root->clipElementId) {
      ...
      if (clipHashMapItem && !Clay__ElementIsOffscreen(&clipHashMapItem->boundingBox)) {
        Clay__AddRenderCommand(CLAY__INIT(Clay_RenderCommand){
            .id = Clay__HashNumber(rootElement->id, rootElement->children.length + 11).id,
            .commandType = CLAY_RENDER_COMMAND_TYPE_SCISSOR_END});
      }
    }
  }
...
Clay__DebugGetElementConfigTypeLabel(Clay__DebugElementConfigType type) {
  ...
  case CLAY__ELEMENT_CONFIG_TYPE_BACKGROUND_COLOR:
    return CLAY__INIT(Clay__DebugElementConfigTypeLabelConfig){CLAY_STRING("Background"),
                                                               {243, 134, 48, 255}};
  case CLAY__ELEMENT_CONFIG_TYPE_OVERLAY_COLOR:
    return CLAY__INIT(Clay__DebugElementConfigTypeLabelConfig){CLAY_STRING("Overlay"),
                                                               {142, 129, 206, 255}};
  case CLAY__ELEMENT_CONFIG_TYPE_CORNER_RADIUS:
    return CLAY__INIT(Clay__DebugElementConfigTypeLabelConfig){CLAY_STRING("Radius"),
                                                               {239, 148, 157, 255}};
...
void Clay__RenderElementConfigTypeLabel(Clay_String label, Clay_Color color, bool offscreen) {
  ...
  CLAY_AUTO_ID({.layout = {.padding = {8, 8, 2, 2}},
    ...
    CLAY_TEXT(label, CLAY_TEXT_CONFIG({.textColor = offscreen ? CLAY__DEBUGVIEW_COLOR_3
  ...
  for (int32_t rootIndex = 0; rootIndex < initialRootsLength; ++rootIndex) {
    ...
    if (rootIndex > 0) {
      CLAY(CLAY_IDI("Clay__DebugView_EmptyRowOuter", rootIndex),
        ...
        CLAY(
            CLAY_IDI("Clay__DebugView_EmptyRow", rootIndex),
            {.layout = {.sizing = {.width = CLAY_SIZING_GROW(0),
    ...
    while (dfsBuffer.length > 0) {
      ...
      CLAY(CLAY_IDI("Clay__DebugView_ElementOuter", currentElement->id),
        ...
        if (currentElementData->elementId.stringId.length > 0) {
          CLAY_AUTO_ID() {
            Clay_TextElementConfig textConfig =
                offscreen ? CLAY__INIT(Clay_TextElementConfig){.textColor = CLAY__DEBUGVIEW_COLOR_3,
                                                               .fontSize = 16}
        ...
        if (currentElement->isTextElement) {
          Clay__RenderElementConfigTypeLabel(CLAY_STRING("Text"),
                                             CLAY__INIT(Clay_Color){105, 210, 231, 255}, offscreen);
        } else {
          if (currentElement->config.backgroundColor.a > 0) {
            Clay__DebugElementConfigTypeLabelConfig config =
                Clay__DebugGetElementConfigTypeLabel(CLAY__ELEMENT_CONFIG_TYPE_BACKGROUND_COLOR);
            Clay__RenderElementConfigTypeLabel(config.label, config.color, offscreen);
          }
          if (currentElement->config.overlayColor.a > 0) {
            Clay__DebugElementConfigTypeLabelConfig config =
                Clay__DebugGetElementConfigTypeLabel(CLAY__ELEMENT_CONFIG_TYPE_OVERLAY_COLOR);
            Clay__RenderElementConfigTypeLabel(config.label, config.color, offscreen);
          }
          if (!Clay__MemCmp((const char *)&currentElement->config.cornerRadius,
                            (const char *)&Clay__CornerRadius_DEFAULT, sizeof(Clay_CornerRadius))) {
            Clay__DebugElementConfigTypeLabelConfig config =
                Clay__DebugGetElementConfigTypeLabel(CLAY__ELEMENT_CONFIG_TYPE_CORNER_RADIUS);
            Clay__RenderElementConfigTypeLabel(config.label, config.color, offscreen);
          }
          if (currentElement->config.aspectRatio.aspectRatio != 0) {
            Clay__DebugElementConfigTypeLabelConfig config =
                Clay__DebugGetElementConfigTypeLabel(CLAY__ELEMENT_CONFIG_TYPE_ASPECT);
            Clay__RenderElementConfigTypeLabel(config.label, config.color, offscreen);
          }
          if (currentElement->config.image.imageData) {
            Clay__DebugElementConfigTypeLabelConfig config =
                Clay__DebugGetElementConfigTypeLabel(CLAY__ELEMENT_CONFIG_TYPE_IMAGE);
            Clay__RenderElementConfigTypeLabel(config.label, config.color, offscreen);
          }
          if (currentElement->config.floating.attachTo != CLAY_ATTACH_TO_NONE) {
            Clay__DebugElementConfigTypeLabelConfig config =
                Clay__DebugGetElementConfigTypeLabel(CLAY__ELEMENT_CONFIG_TYPE_FLOATING);
            Clay__RenderElementConfigTypeLabel(config.label, config.color, offscreen);
          }
          if (currentElement->config.clip.horizontal || currentElement->config.clip.vertical) {
            Clay__DebugElementConfigTypeLabelConfig config =
                Clay__DebugGetElementConfigTypeLabel(CLAY__ELEMENT_CONFIG_TYPE_CLIP);
            Clay__RenderElementConfigTypeLabel(config.label, config.color, offscreen);
          }
          if (Clay__BorderHasAnyWidth(&currentElement->config.border)) {
            Clay__DebugElementConfigTypeLabelConfig config =
                Clay__DebugGetElementConfigTypeLabel(CLAY__ELEMENT_CONFIG_TYPE_BORDER);
            Clay__RenderElementConfigTypeLabel(config.label, config.color, offscreen);
          }
          if (currentElement->config.custom.customData) {
            Clay__DebugElementConfigTypeLabelConfig config =
                Clay__DebugGetElementConfigTypeLabel(CLAY__ELEMENT_CONFIG_TYPE_CUSTOM);
            Clay__RenderElementConfigTypeLabel(config.label, config.color, offscreen);
          }
        }
      }
...
void Clay__DebugViewRenderElementConfigHeader(Clay_String elementId,
...
void Clay__RenderDebugView(void) {
  ...
  CLAY(CLAY_ID("Clay__DebugView"),
    ...
    CLAY_AUTO_ID(
        {.layout = {.sizing = {CLAY_SIZING_GROW(0), CLAY_SIZING_FIXED(CLAY__DEBUGVIEW_ROW_HEIGHT)},
      ...
      CLAY_AUTO_ID(
        ...
        if (selectedItem->layoutElement->isTextElement) {
          ...
          CLAY_AUTO_ID({.layout = {.padding = attributeConfigPadding,
                                   .childGap = 8,
                                   .layoutDirection = CLAY_TOP_TO_BOTTOM}}) {
            Clay__DebugViewRenderElementConfigHeader(selectedItem->elementId.stringId,
                                                     CLAY__ELEMENT_CONFIG_TYPE_TEXT);
            // .fontSize
            ...
            CLAY_TEXT(textConfig->lineHeight == 0 ? CLAY_STRING("auto")
          ...
          CLAY(CLAY_ID("Clay__DebugViewElementInfoSharedBody"),
            ...
            if (selectedItem->layoutElement->config.backgroundColor.a > 0) {
              ...
              Clay__RenderDebugViewColor(selectedItem->layoutElement->config.backgroundColor,
                                         infoTextConfig);
            }
            // .cornerRadius
            if (!Clay__MemCmp((const char *)&selectedItem->layoutElement->config.cornerRadius,
                              (const char *)&Clay__CornerRadius_DEFAULT,
                              sizeof(Clay_CornerRadius))) {
              CLAY_TEXT(CLAY_STRING("Corner Radius"), infoTitleConfig);
              Clay__RenderDebugViewCornerRadius(selectedItem->layoutElement->config.cornerRadius,
                                                infoTextConfig);
            }
            // .overlayColor
            if (selectedItem->layoutElement->config.overlayColor.a > 0) {
              CLAY_TEXT(CLAY_STRING("Overlay Color"), infoTitleConfig);
              Clay__RenderDebugViewColor(selectedItem->layoutElement->config.overlayColor,
                                         infoTextConfig);
            }
          }
          ...
          if (selectedItem->layoutElement->config.image.imageData) {
            ...
            CLAY(CLAY_ID("Clay__DebugViewElementInfoImageBody"),
              ...
              CLAY_AUTO_ID({.layout = {.sizing = {.width = CLAY_SIZING_GROW(64, 128),
          ...
          if (clipConfig->horizontal || clipConfig->vertical) {
            CLAY_AUTO_ID({.layout = {.padding = attributeConfigPadding,
              ...
              CLAY_TEXT(clipConfig->vertical ? CLAY_STRING("true") : CLAY_STRING("false"),
                        infoTextConfig);
              // .horizontal
              CLAY_TEXT(CLAY_STRING("Horizontal"), infoTitleConfig);
              CLAY_TEXT(clipConfig->horizontal ? CLAY_STRING("true") : CLAY_STRING("false"),
                        infoTextConfig);
            }
          }
...
Clay__WarningArray Clay__WarningArray_Allocate_Arena(int32_t capacity, Clay_Arena *arena) {
  ...
  if (nextAllocOffset + totalSizeBytes <= arena->capacity) {
    ...
    Clay__currentContext->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData){
        ...
        .errorText =
            CLAY_STRING("Clay attempted to allocate memory in its arena, but ran out of capacity. "
                        "Try increasing the capacity of the arena passed to Clay_Initialize()"),
  ...
  for (int32_t rootIndex = context->layoutElementTreeRoots.length - 1; rootIndex >= 0;
    ...
    while (dfsBuffer.length > 0) {
      ...
      Clay_LayoutElementHashMapItem *mapItem = Clay__GetHashMapItem(
          currentElement
              ->id); // TODO think of a way around this, maybe the fact that it's essentially a
                     // binary tree limits the cost, but the worst case is not great
            ...
            if (data->elementId == currentElement->id) {
              if (currentElement->config.transition.interactionHandling ==
                ...
                if (data->state == CLAY_TRANSITION_STATE_EXITING ||
                    ...
                    ((data->activeProperties & CLAY_TRANSITION_PROPERTY_POSITION) &&
                     data->state == CLAY_TRANSITION_STATE_TRANSITIONING)) {
                  skipTree = true;
                }
              } else if (currentElement->config.transition.interactionHandling ==
                         CLAY_TRANSITION_ALLOW_INTERACTIONS_WHILE_TRANSITIONING_POSITION) {
                if (data->state == CLAY_TRANSITION_STATE_EXITING) {
                  skipTree = true;
                }
    ...
    if (found && rootElement->config.floating.attachTo != CLAY_ATTACH_TO_NONE &&
        rootElement->config.floating.pointerCaptureMode == CLAY_POINTER_CAPTURE_MODE_CAPTURE) {
      break;
    }
  }
...
void Clay__CloneElementsWithExitTransition() {
  ...
  for (int i = 0; i < context->transitionDatas.length; ++i) {
    ...
    if (data->transitionOut) {
      ...
      bfsBuffer.length = 0;
      Clay_LayoutElement *newElement = Clay_LayoutElementArray_Set_DontTouchLength(
      ...
      Clay__StringArray_Set_DontTouchLength(
          ...
          *Clay__StringArray_GetCheckCapacity(&context->layoutElementIdStrings,
      ...
      while (bufferIndex < bfsBuffer.length) {
        Clay_LayoutElement *layoutElement = Clay_LayoutElementArray_GetCheckCapacity(
            &context->layoutElements, Clay__int32_tArray_GetValue(&bfsBuffer, bufferIndex));
        bufferIndex++;
        for (int j = layoutElement->children.length - 1; j >= 0; --j) {
          Clay_LayoutElement *childElement = Clay_LayoutElementArray_GetCheckCapacity(
              &context->layoutElements, layoutElement->children.elements[j]);
          Clay__int32_tArray_Add(&bfsBuffer, nextIndex);
          Clay_LayoutElement *newChildElement = Clay_LayoutElementArray_Set_DontTouchLength(
          ...
          Clay__StringArray_Set_DontTouchLength(
              ...
              *Clay__StringArray_GetCheckCapacity(&context->layoutElementIdStrings,
                                                  childElement -
                                                      context->layoutElements.internalArray));
          Clay__int32_tArray_Set_DontTouchLength(&context->layoutElementChildren, nextChildIndex,
                                                 nextIndex);
          nextIndex--;
          nextChildIndex--;
        }
...
void Clay_ApplyTransitionedPropertiesToElement(Clay_LayoutElement *currentElement,
                                               Clay_TransitionProperty properties,
                                               Clay_TransitionData currentTransitionData,
                                               Clay_BoundingBox *boundingBox, bool reparented) {
...
Clay_RenderCommandArray Clay_EndLayout(float deltaTime) {
  ...
  if (context->openLayoutElementStack.length > 1) {
    context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData){
        ...
        .errorText = CLAY_STRING(
            "There were still open layout elements when EndLayout was called. This results from an "
            "unequal number of calls to Clay__OpenElement and Clay__CloseElement."),
        .userData = context->errorHandler.userData});
  }
  ...
  for (int i = 0; i < context->transitionDatas.length; ++i) {
    ...
    if (!data->transitionOut && (hashMapItem->generation <= context->generation ||
  ...
  for (int i = 0; i < context->transitionDatas.length; ++i) {
    Clay__TransitionDataInternal *data =
        Clay__TransitionDataInternalArray_Get(&context->transitionDatas, i);
    Clay_LayoutElementHashMapItem *hashMapItem = Clay__GetHashMapItem(data->elementId);
    // This might seems strange - can't we just look up the element itself, and check the config to
    // see whether it has an exit transition defined? That would work fine if the element actually
    // had an exit transition in the first place. If it doesn't have an exit transition defined, the
    // element will have simply disappeared completely at this point, and there will be no element
    // through which to access the config.
    if (data->transitionOut) {
      Clay_TransitionElementConfig *config = &data->elementThisFrame->config.transition;
      ...
      if (hashMapItem->generation <= context->generation) {
        ...
        if (config->exit.trigger == CLAY_TRANSITION_EXIT_TRIGGER_WHEN_PARENT_EXITS ||
            !parentHashMapItem || parentHashMapItem->generation > context->generation) {
          ...
          if (data->state != CLAY_TRANSITION_STATE_EXITING) {
            if (parentHashMapItem->generation <= context->generation) {
              ...
              data->elementThisFrame->config.floating.offset =
                  CLAY__INIT(Clay_Vector2){hashMapItem->boundingBox.x, hashMapItem->boundingBox.y};
          ...
          bfsBuffer.length = 0;
          data->elementThisFrame =
              Clay_LayoutElementArray_Add(&context->layoutElements, *data->elementThisFrame);
          int32_t exitingElementIndex =
              data->elementThisFrame - context->layoutElements.internalArray;
          Clay__StringArray_Add(&context->layoutElementIdStrings,
          ...
          while (bufferIndex < bfsBuffer.length) {
            ...
            // Children of exiting elements may have been moved elsewhere in the layout, this
            // prevents a duplicate ID error if they still exist.
              ...
              for (int j = 0; j < layoutElement->children.length; ++j) {
                ...
                if (childMapItem->generation <= context->generation) {
                  ...
                  Clay_LayoutElement *newChildElement =
                      Clay_LayoutElementArray_Add(&context->layoutElements, *childElement);
                  Clay__StringArray_Add(&context->layoutElementIdStrings,
                                        *Clay__StringArray_GetCheckCapacity(
                                            &context->layoutElementIdStrings, childElementIndex));
                  Clay__int32_tArray_Add(
                      &context->layoutElementClipElementIds,
                      *Clay__int32_tArray_GetCheckCapacity(&context->layoutElementClipElementIds,
                                                           childElementIndex));
          ...
          if (parentHashMapItem->generation > context->generation &&
            ...
            for (int j = 0; j < parentElement->children.length; ++j) {
              if (config->exit.siblingOrdering == CLAY_EXIT_TRANSITION_ORDERING_NATURAL_ORDER &&
                  j == data->siblingIndex) {
                Clay__int32_tArray_Add(&context->layoutElementChildren, exitingElementIndex);
                found = true;
              }
            ...
            parentElement->children.elements =
                &context->layoutElementChildren.internalArray[newChildrenStartIndex];
            // Otherwise, create the tree root for the floating element (needs to be created every
            // frame)
          } else {
            Clay__LayoutElementTreeRootArray_Add(
                &context->layoutElementTreeRoots,
                CLAY__INIT(Clay__LayoutElementTreeRoot){
                    .layoutElementIndex =
                        (int32_t)(data->elementThisFrame - context->layoutElements.internalArray),
                    .parentId = floatingConfig->parentId,
                    .zIndex = floatingConfig->zIndex,
                });
          }
  ...
  for (int i = 0; i < elementIdsToRemoveTransitions.length; ++i) {
    for (int j = 0; j < context->transitionDatas.length; ++j) {
      if (Clay__TransitionDataInternalArray_Get(&context->transitionDatas, j)->elementId ==
      ...
      for (int i = 0; i < context->transitionDatas.length; ++i) {
        ...
        if (mapItem->appearedThisFrame) {
          if (currentElement->config.transition.enter.setInitialState &&
            ...
            transitionData->initialState = currentElement->config.transition.enter.setInitialState(
                transitionData->targetState, currentElement->config.transition.properties);
            ...
            Clay_ApplyTransitionedPropertiesToElement(
                ...
                transitionData->initialState, &mapItem->boundingBox, transitionData->reparented);
          } else {
          ...
          if (transitionData->state != CLAY_TRANSITION_STATE_EXITING) {
            ...
            if (properties & CLAY_TRANSITION_PROPERTY_X) {
              ...
              if (
                  ...
                  // The element is still in the same parent container, and it's parent-relative
                  // position didn't change (parent moved)
                  && (!(Clay__FloatEqual(oldRelativePosition.x, newRelativePosition.x)) ||
            ...
            if (properties & CLAY_TRANSITION_PROPERTY_Y) {
              ...
              if (!Clay__FloatEqual(oldTargetState.boundingBox.y, targetState.boundingBox.y) &&
                  (!(Clay__FloatEqual(oldRelativePosition.y, newRelativePosition.y)) ||
                   transitionData->reparented) &&
                  !context->rootResizedLastFrame) {
            ...
            if (newActiveProperties != 0) {
              ...
              transitionData->activeProperties =
                  (Clay_TransitionProperty)(transitionData->activeProperties | newActiveProperties);
            }
          }
          ...
          if (transitionData->state == CLAY_TRANSITION_STATE_IDLE) {
            ...
            transitionComplete = currentElement->config.transition.handler(
                CLAY__INIT(Clay_TransitionCallbackArguments){
                    ...
                    &transitionData->currentState, targetState, transitionData->elapsedTime,
                    currentElement->config.transition.duration, transitionData->activeProperties});

            Clay_ApplyTransitionedPropertiesToElement(
                currentElement, transitionData->activeProperties, transitionData->currentState,
                &mapItem->boundingBox, transitionData->reparented);
  ...
  for (int i = 0; i < context->layoutElementsHashMap.capacity; ++i) {
    ...
    while (currentElementIndex != -1) {
      Clay_LayoutElementHashMapItem *currentItem = Clay__LayoutElementHashMapItemArray_Get(
      ...
      if (currentItem->generation <= context->generation) {
        ...
        if (previousElementIndex == -1) {
          ...
          Clay_LayoutElementHashMapItem *previousItem = Clay__LayoutElementHashMapItemArray_Get(
              &context->layoutElementsHashMapInternal, previousElementIndex);
...
Clay_ScrollContainerData Clay_GetScrollContainerData(Clay_ElementId id) {
  ...
  for (int32_t i = 0; i < context->scrollContainerDatas.length; ++i) {
    ...
    if (scrollContainerData->elementId == id.id) {
      if (!scrollContainerData->layoutElement) { // This can happen on the first frame before a
...
CLAY_DLL_EXPORT bool Clay_EaseOut(Clay_TransitionCallbackArguments arguments) {
  ...
  if (arguments.properties & CLAY_TRANSITION_PROPERTY_X) {
    arguments.current->boundingBox.x =
        CLAY__LERP(arguments.initial.boundingBox.x, arguments.target.boundingBox.x, lerpAmount);
  }
  if (arguments.properties & CLAY_TRANSITION_PROPERTY_Y) {
    arguments.current->boundingBox.y =
        CLAY__LERP(arguments.initial.boundingBox.y, arguments.target.boundingBox.y, lerpAmount);
  }
  if (arguments.properties & CLAY_TRANSITION_PROPERTY_WIDTH) {
    arguments.current->boundingBox.width = CLAY__LERP(
        arguments.initial.boundingBox.width, arguments.target.boundingBox.width, lerpAmount);
  }
  if (arguments.properties & CLAY_TRANSITION_PROPERTY_HEIGHT) {
    arguments.current->boundingBox.height = CLAY__LERP(
        arguments.initial.boundingBox.height, arguments.target.boundingBox.height, lerpAmount);
  }
  if (arguments.properties & CLAY_TRANSITION_PROPERTY_BACKGROUND_COLOR) {
    arguments.current->backgroundColor = CLAY__INIT(Clay_Color){
        .r = CLAY__LERP(arguments.initial.backgroundColor.r, arguments.target.backgroundColor.r,
                        lerpAmount),
        .g = CLAY__LERP(arguments.initial.backgroundColor.g, arguments.target.backgroundColor.g,
                        lerpAmount),
        .b = CLAY__LERP(arguments.initial.backgroundColor.b, arguments.target.backgroundColor.b,
                        lerpAmount),
        .a = CLAY__LERP(arguments.initial.backgroundColor.a, arguments.target.backgroundColor.a,
                        lerpAmount),
    };
  }
  if (arguments.properties & CLAY_TRANSITION_PROPERTY_OVERLAY_COLOR) {
    arguments.current->overlayColor = CLAY__INIT(Clay_Color){
        .r = CLAY__LERP(arguments.initial.overlayColor.r, arguments.target.overlayColor.r,
                        lerpAmount),
        .g = CLAY__LERP(arguments.initial.overlayColor.g, arguments.target.overlayColor.g,
                        lerpAmount),
        .b = CLAY__LERP(arguments.initial.overlayColor.b, arguments.target.overlayColor.b,
                        lerpAmount),
        .a = CLAY__LERP(arguments.initial.overlayColor.a, arguments.target.overlayColor.a,
                        lerpAmount),
    };
  }
  if (arguments.properties & CLAY_TRANSITION_PROPERTY_BORDER_COLOR) {
    arguments.current->borderColor = CLAY__INIT(Clay_Color){
        .r =
            CLAY__LERP(arguments.initial.borderColor.r, arguments.target.borderColor.r, lerpAmount),
        .g =
            CLAY__LERP(arguments.initial.borderColor.g, arguments.target.borderColor.g, lerpAmount),
        .b =
            CLAY__LERP(arguments.initial.borderColor.b, arguments.target.borderColor.b, lerpAmount),
        .a =
            CLAY__LERP(arguments.initial.borderColor.a, arguments.target.borderColor.a, lerpAmount),
    };
  }
  if (arguments.properties & CLAY_TRANSITION_PROPERTY_BORDER_WIDTH) {
    arguments.current->borderWidth = CLAY__INIT(Clay_BorderWidth){
        .left = (uint16_t)CLAY__LERP(arguments.initial.borderWidth.left,
                                     arguments.target.borderWidth.left, lerpAmount),
        .right = (uint16_t)CLAY__LERP(arguments.initial.borderWidth.right,
                                      arguments.target.borderWidth.right, lerpAmount),
        .top = (uint16_t)CLAY__LERP(arguments.initial.borderWidth.top,
                                    arguments.target.borderWidth.top, lerpAmount),
        .bottom = (uint16_t)CLAY__LERP(arguments.initial.borderWidth.bottom,
                                       arguments.target.borderWidth.bottom, lerpAmount),
        .betweenChildren =
            (uint16_t)CLAY__LERP(arguments.initial.borderWidth.betweenChildren,
