...
// all rendering is performed by a single SDL call, avoiding multiple RenderRect + plumbing choice
// for circles.
static void SDL_Clay_RenderFillRoundedRect(Clay_SDL3RendererData *rendererData,
                                           const SDL_FRect rect, const float cornerRadius,
  ...
  vertices[vertexCount++] =
      (SDL_Vertex){{rect.x + rect.w - clampedRadius, rect.y + rect.h - clampedRadius},
                   color,
                   {1, 1}}; // 2 center BR
  vertices[vertexCount++] = (SDL_Vertex){
  ...
  vertices[vertexCount++] =
      (SDL_Vertex){{rect.x + rect.w - clampedRadius, rect.y}, color, {1, 0}}; // TR
  ...
  // Right edge
  vertices[vertexCount++] =
      (SDL_Vertex){{rect.x + rect.w, rect.y + clampedRadius}, color, {1, 0}}; // RT
  vertices[vertexCount++] =
  ...
  vertices[vertexCount++] =
      (SDL_Vertex){{rect.x + clampedRadius, rect.y + rect.h}, color, {0, 1}}; // BL
  ...
  // Left edge
  vertices[vertexCount++] =
      (SDL_Vertex){{rect.x, rect.y + rect.h - clampedRadius}, color, {0, 1}};              // LB
...
static void SDL_Clay_RenderArc(Clay_SDL3RendererData *rendererData, const SDL_FPoint center,
  ...
  const int numCircleSegments = SDL_max(
      NUM_CIRCLE_SEGMENTS,
      (int)(radius * 1.5F)); // increase circle segments for larger circles, 1.5 is arbitrary.

  const float angleStep = (radEnd - radStart) / (float)numCircleSegments;
  const float thicknessStep = 0.4F; // arbitrary value to avoid overlapping lines. Changing
...
void SDL_Clay_RenderClayCommands(Clay_SDL3RendererData *rendererData,
  ...
  for (size_t i = 0; i < rcommands->length; i++) {
    ...
    const SDL_FRect rect = {(int)bounding_box.x, (int)bounding_box.y, (int)bounding_box.width,
    ...
    case CLAY_RENDER_COMMAND_TYPE_RECTANGLE: {
      ...
      if (config->cornerRadius.topLeft > 0) {
        SDL_Clay_RenderFillRoundedRect(rendererData, rect, config->cornerRadius.topLeft,
    ...
    case CLAY_RENDER_COMMAND_TYPE_TEXT: {
      ...
      TTF_Text *text = TTF_CreateText(rendererData->textEngine, font, config->stringContents.chars,
                                      config->stringContents.length);
      TTF_SetTextColor(text, config->textColor.r, config->textColor.g, config->textColor.b,
                       config->textColor.a);
    ...
    case CLAY_RENDER_COMMAND_TYPE_BORDER: {
      ...
      // edges
      SDL_SetRenderDrawColor(rendererData->renderer, config->color.r, config->color.g,
